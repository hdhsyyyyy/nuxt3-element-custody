...
async function main() {
  ...
  for (const locale of listLocaleToTranslate) {
    ...
    try {
      ...
      // keep the translation and merge
      const targetFile = yaml.load(fs.readFileSync(locale.path, 'utf8'))
      const m = merge(t, targetFile)
      // save to file
      fs.writeFileSync(locale.path, yaml.dump(m))
    } catch (e) {
      console.error(e)
    }
  }
